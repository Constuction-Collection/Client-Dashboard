...
const app = express();
app.use(cors());
// Limit raised so base64-encoded client photo uploads fit
...
app.get('/api/client-projects-with-types', async (req, res) => {
  try {
    ...
    projectsTableResponse.data.records.forEach((proj) => {
      const clientContactIds = proj.fields['Client Contact'] || [];
      const isContactMatch = clientContactIds.includes(contactId);
...
app.post('/api/upload-product-photo', async (req, res) => {
  try {
    ...
    // Airtable's upload endpoint appends to the field, so existing photos are never replaced
    await axios.post(
      `https://content.airtable.com/v0/${BASE_ID}/${productRecordId}/${encodeURIComponent('Client Photos')}/uploadAttachment`,
      {
        contentType: contentType || 'image/jpeg',
        file: file,
        filename: filename || 'client-photo.jpg'
      },
      {
        headers: {
          'Authorization': `Bearer ${AIRTABLE_API_TOKEN}`,
    ...
    // Re-read the record so the client gets back the full, current photo list
